<template>
    <v-dialog :persistent="!!persistent" v-model="showState">
        <v-card class="pa-6 pa-sm-10 pa-md-12">
            <template #title>
                <div class="d-flex align-center justify-center">
                    ...
                    <v-btn
                        ...
                        v-if="
                            currentStep === 'defineColumn' &&
                            importTransactionDefineColumnTab?.menus
                        "
                    >
                        ...
                        <v-menu activator="parent" max-height="500">
                            <v-list>
                                <v-list-item
                                    ...
                                    v-for="(
                                        menu, index
                                    ) in importTransactionDefineColumnTab.menus"
                                />
                            </v-list>
                        </v-menu>
                    </v-btn>
                    <v-btn
                        ...
                        :disabled="loading || submitting"
                        v-if="
                            currentStep === 'executeCustomScript' &&
                            importTransactionExecuteCustomScriptTab?.menus
                        "
                    >
                        <v-icon :icon="mdiDotsVertical" />
                        <v-menu activator="parent" max-height="500">
                            <v-list>
                                <v-list-item
                                    ...
                                    v-for="(
                                        menu, index
                                    ) in importTransactionExecuteCustomScriptTab.menus"
                                />
                            </v-list>
                        </v-menu>
                    </v-btn>
                    <v-btn
                        ...
                        :disabled="loading || submitting"
                        v-if="
                            currentStep === 'checkData' &&
                            importTransactionCheckDataTab?.filterMenus
                        "
                    >
                        ...
                        <v-menu activator="parent" max-height="500">
                            <v-list>
                                <template
                                    ...
                                    v-for="(
                                        group, groupIndex
                                    ) in importTransactionCheckDataTab.filterMenus"
                                >
                    ...
                    <v-btn
                        ...
                        v-if="
                            currentStep === 'checkData' &&
                            importTransactionCheckDataTab?.toolMenus
                        "
                    >
                        <v-icon :icon="mdiDotsVertical" />
                        <v-menu activator="parent" max-height="500">
                            <v-list>
                                <template
                                    ...
                                    v-for="(
                                        menu, index
                                    ) in importTransactionCheckDataTab.toolMenus"
                                >
                                    <v-divider
                                        class="my-2"
                                        v-if="menu.divider"
            ...
            <v-window class="disable-tab-transition" v-model="currentStep">
                <v-window-item value="uploadFile">
                    <v-row>
                        <v-col cols="12" md="12">
                            <two-column-select
                                primary-key-field="displayCategoryName"
                                ...
                                :filter-no-items-text="
                                    tt('No available file type')
                                "
                                :label="tt('File Type')"
                        ...
                        <v-col
                            cols="12"
                            md="12"
                            v-if="fileType === 'dsv' || fileType === 'dsv_data'"
                        >
                            <v-select
                                item-title="displayName"
                                item-value="type"
                                ...
                                :items="[
                                    {
                                        displayName: tt('Column Mapping'),
                                        type: ImportDSVProcessMethod.ColumnMapping,
                                    },
                                    {
                                        displayName: tt('Custom Script'),
                                        type: ImportDSVProcessMethod.CustomScript,
                                    },
                                ]"
                        ...
                        >
                            <v-text-field
                                ...
                                :label="tt('Data File')"
                                :placeholder="
                                    tt('format.misc.clickToSelectedFile', {
                                        extensions:
                        ...
                        <v-col
                            ...
                            md="12"
                            class="mb-0 pb-0"
                            v-if="exportFileGuideDocumentUrl"
                        >
                            <a
                                :href="exportFileGuideDocumentUrl"
                            ...
                            >
                                ...
                                <span
                                    ...
                                    v-if="
                                        ...
                                        fileType === 'dsv_data'
                                    "
                                    >{{ tt("How to import this file?") }}</span
                                >
                                <span
                                    class="ms-1"
                                    v-if="
                                        ...
                                        fileType !== 'dsv_data'
                                    "
                                    >{{ tt("How to export this file?") }}</span
                                >
                                <span
                                    class="ms-1"
                                    v-if="exportFileGuideDocumentLanguageName"
                                    >[{{
                ...
                <v-window-item value="finalResult">
                    ...
                    <p class="my-5">
                        {{
                            tt("format.misc.importTransactionResult", {
                                count: getDisplayCount(importedCount || 0),
                            })
                        }}
                    </p>
                </v-window-item>
            ...
            <div
                class="d-flex justify-sm-space-between gap-4 flex-wrap justify-center mt-10"
            >
                <v-btn
                    color="secondary"
                    variant="tonal"
                ...
                <v-btn
                    ...
                    :disabled="
                        loading ||
                        submitting ||
                        (!isImportDataFromTextbox && !importFile) ||
                        (isImportDataFromTextbox && !importData)
                    "
                    :append-icon="!submitting ? mdiArrowRight : undefined"
                    @click="parseData"
                    v-if="
                        currentStep === 'defineColumn' ||
                        currentStep === 'executeCustomScript' ||
                ...
                <v-btn
                    ...
                    :disabled="
                        submitting ||
                        importTransactionCheckDataTab?.isEditing ||
                        !importTransactionCheckDataTab?.canImport
                    "
                    :append-icon="!submitting ? mdiArrowRight : undefined"
                    @click="submit"
                ...
                >
                    {{
                        submitting && importProcess > 0
                            ? tt("format.misc.importingTransactions", {
                                  process: formatNumberToLocalizedNumerals(
                                      importProcess,
                                      2
                                  ),
                              })
    ...
    <input
        ...
        type="file"
        style="display: none"
        :accept="supportedImportFileExtensions"
...
import type {
    ...
    LocalizedImportFileType,
    LocalizedImportFileTypeSubType,
    LocalizedImportFileTypeSupportedEncodings,
} from "@/core/file.ts";
...
type ImportTransactionDefineColumnTabType = InstanceType<
    typeof ImportTransactionDefineColumnTab
>;
type ImportTransactionExecuteCustomScriptTabType = InstanceType<
    typeof ImportTransactionExecuteCustomScriptTab
>;
type ImportTransactionCheckDataTabType = InstanceType<
    typeof ImportTransactionCheckDataTab
>;
...
const importTransactionDefineColumnTab =
    useTemplateRef<ImportTransactionDefineColumnTabType>(
        "importTransactionDefineColumnTab"
    );
const importTransactionExecuteCustomScriptTab =
    useTemplateRef<ImportTransactionExecuteCustomScriptTabType>(
        "importTransactionExecuteCustomScriptTab"
    );
const importTransactionCheckDataTab =
    useTemplateRef<ImportTransactionCheckDataTabType>(
        "importTransactionCheckDataTab"
    );
...
const processDSVMethod = ref<ImportDSVProcessMethod>(
    ImportDSVProcessMethod.ColumnMapping
);
...
const numeralSystem = computed<NumeralSystem>(() =>
    getCurrentNumeralSystemType()
);

const allSupportedImportFileCategoryAndTypes = computed<
    LocalizedImportFileCategoryAndTypes[]
>(() => getAllSupportedImportFileCagtegoryAndTypes());
const allFileSubTypes = computed<LocalizedImportFileTypeSubType[] | undefined>(
    () => allSupportedImportFileTypesMap.value[fileType.value]?.subTypes
);
const allSupportedEncodings = computed<
    LocalizedImportFileTypeSupportedEncodings[] | undefined
>(
    () =>
        allSupportedImportFileTypesMap.value[fileType.value]?.supportedEncodings
);
const isImportDataFromTextbox = computed<boolean>(
    () =>
        allSupportedImportFileTypesMap.value[fileType.value]?.dataFromTextbox ??
        false
);
...
const allSupportedImportFileTypesMap = computed<
    Record<string, LocalizedImportFileType>
>(() => {
...
const supportedImportFileExtensions = computed<string | undefined>(() => {
    if (allFileSubTypes.value && allFileSubTypes.value.length) {
        const subTypeExtensions = findExtensionByType(
...
const exportFileGuideDocumentUrl = computed<string | undefined>(() => {
    const document =
        allSupportedImportFileTypesMap.value[fileType.value]?.document;
    ...
    return `https://ezbookkeeping.zzhao8053.net/${language}export_and_import${anchor}`;
});
...
function parseData(): void {
    ...
    if (!isImportDataFromTextbox.value) {
        ...
        if (type === "custom_csv") {
            uploadFile = KnownFileType.CSV.createFile(
                importData.value,
                "import"
            );
        } else if (type === "custom_tsv") {
            uploadFile = KnownFileType.TSV.createFile(
                importData.value,
                "import"
            );
        } else {
        ...
        // should not happen, but ts would check whether uploadFile has been assigned a value
        snackbar.value?.showMessage("An error occurred");
        return;
    }

    const isDsvFileType: boolean =
        fileType.value === "dsv" || fileType.value === "dsv_data";
    ...
    if (isDsvFileType && currentStep.value === "uploadFile") {
            ...
            .then((response) => {
                if (response && response.length) {
                    if (
                        ...
                        ImportDSVProcessMethod.CustomScript
                    ) {
                        importTransactionExecuteCustomScriptTab.value?.reset();
        ...
        if (
            ...
            const defineColumnResult =
                importTransactionDefineColumnTab.value?.generateResult();
            ...
            amountDigitGroupingSymbol =
                defineColumnResult.amountDigitGroupingSymbol;
            ...
            isDsvFileType &&
            processDSVMethod.value === ImportDSVProcessMethod.CustomScript
        ) {
            const executeCustomScriptResult =
                importTransactionExecuteCustomScriptTab.value?.generateResult();
            ...
            encoding = undefined;
            uploadFile = KnownFileType.JSON.createFile(
                executeCustomScriptResult,
                "import"
            );
        }
        ...
        transactionsStore
            ...
            .then((response) => {
                ...
                if (response.items) {
                    for (const [importTransaction, index] of itemAndIndex(
                        response.items
                    )) {
                        const parsedTransaction = ImportTransaction.of(
                            importTransaction,
                            index
                        );
...
function submit(): void {
    ...
    confirmDialog.value
        ?.open("format.misc.confirmImportTransactions", {
...
watch(fileSubType, (newValue) => {
    let supportedExtensions: string | undefined = findExtensionByType(
        allFileSubTypes.value,
        newValue
    );
    ...
    if (!supportedExtensions) {
        supportedExtensions =
            allSupportedImportFileTypesMap.value[fileType.value]?.extensions;
    }
