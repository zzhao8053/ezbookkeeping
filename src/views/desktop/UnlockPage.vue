    ...
    <div class="layout-wrapper">
        ...
        <v-row no-gutters class="auth-wrapper">
            <v-col
                cols="12"
                md="8"
                class="auth-image-background d-none d-md-flex align-center justify-center position-relative"
            >
                <div class="d-flex auth-img-footer" v-if="!isDarkMode">
                    <v-img
                        class="img-with-direction"
                        src="img/desktop/background.svg"
                    />
                </div>
                <div class="d-flex auth-img-footer" v-if="isDarkMode">
                    <v-img
                        class="img-with-direction"
                        src="img/desktop/background-dark.svg"
                    />
                </div>
                <div class="d-flex align-center justify-center w-100 pt-10">
                    <v-img
                        class="img-with-direction"
                        max-width="600px"
                        src="img/desktop/people3.svg"
                        v-if="!isDarkMode"
                    />
                    <v-img
                        class="img-with-direction"
                        max-width="600px"
                        src="img/desktop/people3-dark.svg"
                ...
                <div class="d-flex align-center justify-center h-100">
                    ...
                    >
                        <v-card-text>
                            ...
                            <p class="mb-0" v-if="isWebAuthnAvailable">
                                {{
                                    tt(
                                        "Please enter your PIN code or use WebAuthn to unlock application"
                                    )
                                }}
                            </p>
                            <p class="mb-0" v-else-if="!isWebAuthnAvailable">
                                {{
                                    tt(
                                        "Please enter your PIN code to unlock application"
                                    )
                                }}
                            </p>
                        </v-card-text>
                        ...
                        <v-card-text class="pt-0">
                            <v-row>
                                ...
                                <v-col cols="12" class="text-center text-sm">
                                    ...
                                    <a
                                        href="https://github.com/zzhao8053/ezbookkeeping"
                                        target="_blank"
                                        >ezBookkeeping</a
                                    >&nbsp;<span>{{ version }}</span>
                                </v-col>
...
function unlockByWebAuthn(): void {
    ...
    if (
        !settingsStore.appSettings.applicationLockWebAuthn ||
        !hasWebAuthnConfig()
    ) {
        ...
        .catch((error) => {
            ...
            if (error.notSupported) {
                snackbar.value?.showMessage(
                    "WebAuth is not supported on this device"
                );
                ...
                snackbar.value?.showMessage(
                    "User has canceled or this device does not support WebAuthn"
                );
            }
        });
}
