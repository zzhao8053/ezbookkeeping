<template>
    <f7-page>
            ...
            <f7-nav-right>
                <f7-link
                    ...
                    v-if="
                        ...
                        !forceShowRefreshBrowserCacheMenu
                    "
                />
                <f7-link
                    icon-f7="ellipsis"
                    @click="showRefreshBrowserCacheSheet = true"
                    v-else-if="
                        !clientVersionMatchServerVersion ||
                        forceShowRefreshBrowserCacheMenu
                    "
        ...
        <f7-list strong inset dividers>
            ...
            <f7-list-item
                :title="tt('Build Time')"
                :after="clientBuildTime"
                v-if="clientBuildTime"
            ></f7-list-item>
            <f7-list-item
                external
                :title="tt('Official Website')"
                link="https://github.com/zzhao8053/ezbookkeeping"
                target="_blank"
            ></f7-list-item>
            <f7-list-item
                external
                :title="tt('Report Issue')"
                link="https://github.com/zzhao8053/ezbookkeeping/issues"
                target="_blank"
            ></f7-list-item>
            <f7-list-item
                external
                :title="tt('Getting help')"
                link="https://ezbookkeeping.zzhao8053.net"
                target="_blank"
            ></f7-list-item>
            <f7-list-item
                :title="tt('License')"
                link="#"
                popup-open=".license-popup"
            ></f7-list-item>
        </f7-list>

        <f7-block-title
            class="margin-top"
            v-if="exchangeRatesData && !isUserCustomExchangeRates"
            >{{ tt("Exchange Rates Data") }}</f7-block-title
        >
        <f7-list
            strong
            inset
        ...
        >
            <f7-list-item
                external
                :title="tt('Provider')"
                :after="exchangeRatesData.dataSource"
                :link="exchangeRatesData.referenceUrl"
                target="_blank"
        ...
        <f7-block-title class="margin-top" v-if="mapProviderName">{{
            tt("Map")
        }}</f7-block-title>
        <f7-list strong inset dividers v-if="mapProviderName">
            <f7-list-item
                external
                :title="tt('Provider')"
                :after="mapProviderName"
                :link="mapProviderWebsite"
                target="_blank"
        ...
        >
            <f7-page>
                ...
                <f7-block strong outline class="license-content">
                    ...
                    <p>
                        <span
                            >ezBookkeeping also contains additional third party
                            software and illustration.</span
                        ><br />
                        <span
                            >All the third party software/illustration included
                            or linked is redistributed under the terms and
                            conditions of their original licenses.</span
                        >
                    </p>
                    <p></p>
                    <p
                    ...
                    >
                        ...
                        <br v-if="license.licenseUrl" /><span
                            ...
                            >License: {{ license.licenseUrl }}</span
                        >
                    </p>
                </f7-block>
            </f7-page>
        ...
        <f7-actions
            ...
            close-on-escape
            :opened="showRefreshBrowserCacheSheet"
            @actions:closed="showRefreshBrowserCacheSheet = false"
        >
            <f7-actions-group>
                <f7-actions-button @click="refreshBrowserCache">{{
...
const forceShowRefreshBrowserCacheMenu = computed<boolean>(
    () => versionClickCount.value >= 5
);
...
function showVersion(): void {
    ...
    if (serverDisplayVersion.value) {
        versionMessage += `<br/>${tt("Backend Version")}: ${
            serverDisplayVersion.value
        }`;
    }
    ...
    if (
        serverDisplayVersion.value &&
        serverDisplayVersion.value !== "unknown" &&
        serverDisplayVersion.value !== clientVersion
    ) {
